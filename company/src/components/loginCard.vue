    ...
    <div style="z-index: 100;">
      <div class="title">企业登录</div>
        <el-form
          ...
          label-width="auto"
        >
          <el-form-item label="企业名" prop="username">
...
const FirmRules = reactive({
  username: [{ required: true, message: "请输入企业名", trigger: "blur" }],
...
const submitFirmForm = (formRef) => {
  formRef.validate((valid) => {
    if (valid) {
      axios.post("/companyapi/user/login",FirmForm).then((res) => {
...
const register =()=>{
  axios.post("/companyapi/user/registerCompany",FirmForm).then((res) => {
